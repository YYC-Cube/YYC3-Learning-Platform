...
function getFallbackCourses() {
  return courseData.map((c) => ({
    ...
    price: typeof c.price === "string" ? parseInt(c.price.replace(/[^0-9]/g, ""), 10) || 0 : c.price,
    tags: c.tags,
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      logger.warn("课程数据库查询失败，使用fallback数据:", dbError instanceof Error ? dbError.message : dbError)
    }
    ...
    return NextResponse.json({ success: false, error: "服务器内部错误", message: "获取课程列表失败" }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const field of requiredFields) {
      if (!body[field]) {
        return NextResponse.json({ success: false, error: `缺少必填字段: ${field}`, message: "请填写所有必填信息" }, { status: 400 })
      }
    }
    ...
    try {
      await query(
        ...
        [newCourse.title, newCourse.description, newCourse.category, newCourse.difficulty, newCourse.duration, newCourse.chapters, newCourse.price, JSON.stringify(newCourse.tags), newCourse.instructor, newCourse.image]
      )
    } catch (dbError) {
      logger.warn("课程创建数据库写入失败（fallback）:", dbError instanceof Error ? dbError.message : dbError)
    }

    return NextResponse.json({ success: true, data: newCourse, message: "课程创建成功" }, { status: 201 })
  } catch (error) {
    logger.error("创建课程失败", error)
    return NextResponse.json({ success: false, error: "服务器内部错误", message: "创建课程失败" }, { status: 500 })
  }
}
