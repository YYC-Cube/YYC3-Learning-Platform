...
class QualityAnalyzer {
  analyzeTrends(metrics: QualityMetrics[]): TrendAnalysis {
    // 计算质量趋势
    const functionalTrend = this.calculateTrend(
      metrics.map(m => m.functionalQuality)
    );
    
    const performanceTrend = this.calculateTrend(
      metrics.map(m => m.performanceScore)
    );
    
    const securityTrend = this.calculateTrend(
      metrics.map(m => m.securityLevel)
    );
    ...
    return {
      ...
      recommendations: this.generateRecommendations(functionalTrend, performanceTrend, securityTrend)
    };
  }
